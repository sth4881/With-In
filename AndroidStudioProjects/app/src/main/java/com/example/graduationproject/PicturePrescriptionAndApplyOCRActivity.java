...
public class PicturePrescriptionAndApplyOCRActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pictrue_prescription_and_apply_ocr);

        img = (ImageView)findViewById(R.id.img);

        // 카메라 및 외부 저장소 쓰기 권한 요청
        onRequestPermission();
        // 사진 촬영으로 비트맵 이미지 만들기
    ...
    // 촬영한 처방전에 OCR을 적용 메소드
    private void applyOCR(String encodedImage) {
        ...
        ArrayList<String> ocrData = new ArrayList<String>();
        new Thread() {
            @Override
            public void run() {
                try {
